...
async function ensureLogDir() {
  ...
  try {
    await fs.promises.writeFile(LOG_FILE, `ClickStart log - ${new Date().toISOString()}\n\n`);
  } catch (e) {
    /* ignore */
  }
}
...
async function main() {
  ...
  if (fs.existsSync(pkgPath)) {
    try {
      ...
      log("WARN: package.json exists but could not be parsed");
    }
  } else {
  ...
  if (pkg) {
    const installCmd = fs.existsSync(path.join(REPO_ROOT, "package-lock.json"))
      ? ["npm", ["ci"]]
      : ["npm", ["install"]];
    ...
    if (toInstall.length) {
      ...
      try {
        ...
        log("WARN devDeps install failed: " + e.message);
        await writeReport("DevDeps error", String(e.message));
      }
    } else {
    ...
    const lints = [];
    if (
      fs.existsSync(path.join(REPO_ROOT, ".eslintrc")) ||
      devDeps.eslint ||
      (pkg.scripts && pkg.scripts.lint)
    )
      lints.push({ cmd: "npx", args: ["eslint", ".", "--ext", ".js,.mjs,.cjs,.ts", "--fix"] });
    if (fs.existsSync(path.join(REPO_ROOT, ".stylelintrc")) || devDeps.stylelint)
      lints.push({ cmd: "npx", args: ["stylelint", "**/*.{css,scss}", "--fix"] });
    if (fs.existsSync(path.join(REPO_ROOT, ".htmlhintrc")) || devDeps.htmlhint)
      lints.push({ cmd: "npx", args: ["htmlhint", "."] });
    ...
    for (const lint of lints) {
      ...
      try {
        ...
        log("Lint error: " + e.message);
        await writeReport("Lint error", String(e.message));
      }
    }
...
main().catch(async (err) => {
  ...
  try {
    await writeReport("Unhandled error", String(err));
  } catch (e) {}
  process.exit(1);
});
